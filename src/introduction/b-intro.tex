...
\footnote{Author: D. Higinbotham  (Hall-B version by H.~Avakian \email(avakian@jlab.org))}
}
...
\infolevone{
\section[Hall A Overview]{Hall B Overview}
\label{sec:intro-halla-overview}

The design purpose of Hall B is to study electron scattering 
on nuclei and nucleons at high luminosity
of up to $5\cdot{}10^{36}~\mathrm{cm^{-2}s^{-1}}$ with high momentum
resolution and large acceptance coverage. 


The basic lay-out of Hall B is shown in Fig.~\ref{fig:HallB-side},
...
\begin{figure}[htb]
 \begin{center}
    \includegraphics*[angle=0,width=\textwidth]{HallB}
 \end{center}
\caption[Hall B schematic cross section]%
{Schematic cross section of Hall B. 
}
\label{fig:HallB-side}
\end{figure}
...
liquid hydrogen and polarized $NH_3$ and $ND_3$ targets. Secondary particles
are detected with the CLAS12.  The rest of the beam is transported to the high power
